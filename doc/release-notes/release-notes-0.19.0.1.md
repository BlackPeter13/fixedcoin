FixedCoin Core version 0.19.0.1 is now available from:

  <https://bitcoincore.org/bin/fixedcoin-core-0.19.0.1/>
...
FixedCoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use FixedCoin Core on unsupported systems.
...
to open the FixedCoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the FixedCoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/fixedcoin/fixedcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running FixedCoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of FixedCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `fixedcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile FixedCoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in FixedCoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of FixedCoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in FixedCoin Core,
  ...
  This prevents FixedCoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of FixedCoin Core and nodes that have manually enabled BIP37 support,
...
- By default, FixedCoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `fixedcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how FixedCoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  FixedCoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, FixedCoin Core will be bound to
  the standard port 24768 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 FixedCoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  FIX), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in FIX per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in FixedCoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@FixedCoin AEAD (jonasschnelli)
...
- #15639 fixedcoin-wallet tool: Drop libfixedcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as FIX/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in fixedcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "fixedcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of fixedcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from fixedcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_fixedcoin to auto generated project files (sipsorcery)
...
- #15773 Add FixedCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_fixedcoin output (practicalswift)
...
- #15896 feature_filelock, interface_fixedcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding FixedCoin Core (luke-jr)
...
- #16277 Suppress output in test_fixedcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as FIX/kB (jonatack)
...
- #16489 log: harmonize fixedcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (fixedcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example fixedcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBFIXEDCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update fixedcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in fixedcoin.conf (fanquake)
...
- #16621 add default fixedcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the fixedcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, fixedcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/fixedcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/fixedcoin/fixedcoin/).
