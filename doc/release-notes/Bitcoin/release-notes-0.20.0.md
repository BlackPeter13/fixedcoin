...
- Instead of running `make` simply, you should instead run
  `FIXEDCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from FixedCoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since FixedCoin Core version 0.18.0.
...
- Testing or debugging of implementations of the FixedCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of FixedCoin Core. FixedCoin Core logs debug messages
...
- OpenSSL is no longer used by FixedCoin Core.  (#17265)

- BIP70 support has been fully removed from FixedCoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/fixedcoin/fixedcoin/issues/16599),
  [PR #16702](https://github.com/fixedcoin/fixedcoin/pull/16702), and the
  `fixedcoind help` for more information.  This option is experimental and
...
- All custom settings configured when FixedCoin Core starts are now
...
- The "Start FixedCoin Core on system login" option has been removed on macOS.
  (#17567)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed FixedCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/fixedcoin-core/HWI) for
  signing.  Future versions of FixedCoin Core should support a GUI option
...
- FixedCoin Core's automatically-generated source code documentation is
  now available at https://doxygen.fixedcoincore.org.  (#17596)
...
- The `fixedcoin-cli` utility used with the `-getinfo` parameter now
...
- The `fixedcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=24768` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `fixedcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_fixedcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBFIXEDCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_fixedcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBFIXEDCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for fixedcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate fixedcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest fixedcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_fixedcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (fixedcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone fixedcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about FixedCoin Core files (hebasto)
...
- #17119 Fix broken fixedcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from fixedcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.fixedcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to fixedcoin-wallet (hel-o)
...
- #18341 Replace remaining literal FIX with `CURRENCY_UNIT` (domob1812)
...
- #16115 On fixedcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from fixedcoin-cli -getinfo (malevolent)
