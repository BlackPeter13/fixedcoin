...
def parse_config_into_fix_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"FixedCoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/fixedcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/fixedcoin_config.h'), "w", encoding="utf8") as fix_config:
        fix_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='FixedCoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_fix_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/fixedcoin_config.h'), os.path.join(SOURCE_DIR, 'config/fixedcoin-config.h'))
