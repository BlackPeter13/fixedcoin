...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing fixedcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='fixedcoin-cli', help='fixedcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='FixedCoin address to which the faucet should send')
...
parser.add_argument('fixedcoin_cli_args', nargs='*', help='Arguments to pass on to fixedcoin-cli (default: -signet)')

args = parser.parse_args()

if args.fixedcoin_cli_args == []:
    args.fixedcoin_cli_args = ['-signet']


def fixedcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.fixedcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = fixedcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = fixedcoin_cli(['getnewaddress', 'faucet', 'bech32'])
