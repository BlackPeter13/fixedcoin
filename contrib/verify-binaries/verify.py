...
"""Script for verifying FixedCoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from fixedcoincore.org and fixedcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/fixedcoin-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://fixedcoin.org"
VERSIONPREFIX = "fixedcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the FixedCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"fixedcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by fixedcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the fixedcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes fixedcoin.org lags behind fixedcoincore.org.)')
    )
