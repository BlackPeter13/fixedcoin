...
complete --command fixedcoin-qt --no-files

# Extract options
function __fish_fixedcoinqt_get_options
...
complete \
    --command fixedcoin-qt \
    --arguments "(__fish_fixedcoinqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c fixedcoin-qt \
...
complete \
    --command fixedcoin-qt \
    --arguments "(__fish_fixedcoinqt_get_options --nofiles)"
