...
#include <qt/fixedcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FixedCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new FixedCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FixedCoinAddressCheckValidator(parent));
}
...
bool parseFixedCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no fixedcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("fixedcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!FixedCoinUnits::parse(FixedCoinUnit::FIX, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseFixedCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseFixedCoinURI(uriInstance, out);
}

QString formatFixedCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("fixedcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FixedCoinUnits::format(FixedCoinUnit::FIX, info.amount, false, FixedCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFixedCoinConf()
{
    ...
    /* Open fixedcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FixedCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FixedCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("FixedCoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for FixedCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "fixedcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("fixedcoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a fixedcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=FixedCoin\n";
        else
            optionFile << strprintf("Name=FixedCoin (%s)\n", chain);
