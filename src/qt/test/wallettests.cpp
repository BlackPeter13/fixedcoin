...
#include <key_io.h>
#include <qt/fixedcoinamountfield.h>
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<FixedCoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = FixedCoinUnits::formatWithUnit(unit, expected_balance, false, FixedCoinUnits::SeparatorStyle::ALWAYS);
...
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    FixedCoinAmountField* amountInput = receiveCoinsDialog.findChild<FixedCoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("fixedcoin:"), 2);
