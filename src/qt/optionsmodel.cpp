...
/** Call node.updateRwSetting() with FixedCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because FixedCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/fixedcoin/fixedcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because fixedcoin 22.x will not try to read these.
...
//! Convert enabled/size values to fixedcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from fixedcoin -prune setting.
...
//! Get pruning size value to show in GUI from fixedcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayFixedCoinUnit")) {
        settings.setValue("DisplayFixedCoinUnit", QVariant::fromValue(FixedCoinUnit::FIX));
    }
    QVariant unit = settings.value("DisplayFixedCoinUnit");
    if (unit.canConvert<FixedCoinUnit>()) {
        m_display_fixedcoin_unit = unit.value<FixedCoinUnit>();
    } else {
        m_display_fixedcoin_unit = FixedCoinUnit::FIX;
        settings.setValue("DisplayFixedCoinUnit", QVariant::fromValue(m_display_fixedcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if fixedcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_fixedcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<FixedCoinUnit>() == m_display_fixedcoin_unit) return;
    m_display_fixedcoin_unit = new_unit.value<FixedCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayFixedCoinUnit", QVariant::fromValue(m_display_fixedcoin_unit));
    Q_EMIT displayUnitChanged(m_display_fixedcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/fixedcoin/fixedcoin/pull/8273
    ...
    // (https://github.com/fixedcoin-core/gui/issues/567).
