...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<FixedCoinUnit>("FixedCoinUnit");
#else
    qRegisterMetaType<FixedCoinUnit>("FixedCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. fixedcoin_de.qm (shortcut "de" needs to be defined in fixedcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. fixedcoin_de_DE.qm (shortcut "de_DE" needs to be defined in fixedcoin.qrc)
...
static const char* qt_argv = "fixedcoin-qt";

FixedCoinApplication::FixedCoinApplication()
...
void FixedCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the FixedCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FixedCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void FixedCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool FixedCoinApplication::createOptionsModel(bool resetSettings)
{
...
void FixedCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new FixedCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &FixedCoinGUI::quitRequested, this, &FixedCoinApplication::requestShutdown);
...
void FixedCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void FixedCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool FixedCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void FixedCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &FixedCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &FixedCoinApplication::handleRunawayException);
    connect(this, &FixedCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &FixedCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void FixedCoinApplication::parameterSetup()
{
...
void FixedCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void FixedCoinApplication::requestInitialize()
{
...
void FixedCoinApplication::requestShutdown()
{
...
void FixedCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // fixedcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &FixedCoinGUI::handlePaymentRequest);
            connect(window, &FixedCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void FixedCoinApplication::handleRunawayException(const QString &message)
{
...
void FixedCoinApplication::handleNonFatalException(const QString& message)
{
...
WId FixedCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool FixedCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FixedCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(fixedcoin);
    Q_INIT_RESOURCE(fixedcoin_locale);
    ...
    /// 6-7. Parse fixedcoin.conf, determine network, switch to network specific
    ...
    // fixedcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
