...
const char * const FIXEDCOIN_CONF_FILENAME = "fixedcoin.conf";
const char * const FIXEDCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //fixedcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", FIXEDCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\FixedCoin
    // macOS: ~/Library/Application Support/FixedCoin
    // Unix-like: ~/.fixedcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FixedCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/FixedCoin";
#else
    // Unix-like
    return pathRet / ".fixedcoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", FIXEDCOIN_CONF_FILENAME));
}
